# ==============================================================================
# IPZAN 多账户自动领取工具（每周领IP）
# 功能：IPZAN自动登录 + 每周领取IP，支持多账户 + 双向推送（自定义+青龙）
# IPZAN注册链接：https://www.ipzan.com?pid=18u58r8g
# ==============================================================================
# 一、依赖安装（新环境必装）
# - requests
# ==============================================================================
# 二、环境变量配置（必填）
# 1. 环境变量名：Pzandaili（保持原始名称）
# 2. 格式要求：
#    - 单账户（无推送）：ACCOUNT#PASSWORD
#    - 单账户（带推送）：ACCOUNT#PASSWORD#推送链接（息知/ShowDoc均可）
#    - 多账户：ACCOUNT1#PASSWORD1@ACCOUNT2#PASSWORD2 或 & 分隔
# ==============================================================================
# 三、推送服务说明
# 1. 自定义推送（支持每个账号独立设置）：
#    - 息知推送：关注对应公众号，发送“密钥”获取推送链接（https://xizhi.qqoq.net/xxxx.send）
#    - ShowDoc推送：关注对应公众号，发送“推送”获取推送链接（https://push.showdoc.com.cn/server/api/push/xxxx）
# 2. 青龙面板推送（汇总通知）：
#    - 自动调用青龙面板自带的 notify.py 文件。
#    - 脚本运行结束后，会将所有账号的运行结果汇总，发送一次统一通知。
# ==============================================================================
...
def load_accounts_from_env():
    ...
    for idx, account_str in enumerate(accounts_str_list, 1):
        ...
        acc = parts[0].strip()
        pwd = parts[1].strip()
        # 只有当配置了第三部分且不为空时，才认为是独立推送链接
        push_url = parts[2].strip() if len(parts) >=3 and parts[2].strip() else None
        
        if acc and pwd:
            accounts.append({
                "index": idx, "account": acc, "password": pwd, "push_url": push_url
            })
...
def send_custom_push(push_url, title, content):
    """发送自定义单账户推送（息知/ShowDoc）"""
    if not push_url or not content: return
    try:
        requests.post(
            push_url, headers=PUSH_HEADERS, 
            data={"title": title, "content": content}, 
            timeout=PUSH_TIMEOUT, verify=False
        )
        print("  └─ 自定义推送已发送")
...
def process_single_account(session, account_info):
    ...
    push_url = account_info["push_url"]
    ...
    try:
        ...
        if login_result.get("code") != 0:
            ...
            # 触发独立推送
            if push_url: send_custom_push(push_url, "IPZAN登录失败", summary_msg)
            return summary_msg
        ...
        if code == 0:
            ...
            print(f"  🎉 领取成功：{data}")
            summary_msg = f"账号：{mask_account(account)}\n状态：✅ 领取成功\n结果：{data}"
            if push_url: send_custom_push(push_url, "IPZAN领取成功", summary_msg)
            
        elif code == -1 and "领取过" in str(msg):
            # 重复领取（不算失败）
            print(f"  ⚠️ 本周已领：{msg}")
            summary_msg = f"账号：{mask_account(account)}\n状态：⚠️ 本周已领\n提示：{msg}"
            if push_url: send_custom_push(push_url, "IPZAN领取提示", summary_msg)
            
        else:
            # 其他失败
            print(f"  ❌ 领取失败：{msg}")
            summary_msg = f"账号：{mask_account(account)}\n状态：❌ 领取失败\n原因：{msg}"
            if push_url: send_custom_push(push_url, "IPZAN领取失败", summary_msg)
            
    except Exception as e:
        err_msg = str(e)
        print(f"  ❌ 运行异常：{err_msg}")
        summary_msg = f"账号：{mask_account(account)}\n状态：❌ 运行异常\n原因：{err_msg}"
        if push_url: send_custom_push(push_url, "IPZAN程序异常", summary_msg)
