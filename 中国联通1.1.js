...
- 新增依赖：⚠️ 必须在青龙面板安装 hpagent 依赖，否则无法运行。
...
新增变量(必须):
export UNICOM_PROXY_API="你的品赞JSON提取链接"
⚠️ 注意：API链接中必须包含 &format=json，且在品赞后台勾选“账号密码”授权。
...
// === 修正版：适配 data.list 结构 ===
  async set_proxy_ip() {
    try {
      ...
      this.log("正在通过环境变量获取独立代理IP (JSON模式)...");
      ...
      // === 核心修改在这里 ===
      // 优先匹配你日志中的结构: body.data.list[0]
      if (body.data && body.data.list && Array.isArray(body.data.list) && body.data.list.length > 0) {
      ...
      if (proxyData && proxyData.ip && proxyData.port) {
        const { ip, port, account, password } = proxyData;
        let proxyUrl = "";

        // 构造带账号密码的代理链接
        if (account && password) {
            proxyUrl = `http://${account}:${password}@${ip}:${port}`;
            this.log(`✅ 获取成功(账号密码模式)，IP: ${ip}:${port}`);
        } else {
            proxyUrl = `http://${ip}:${port}`;
            this.log(`✅ 获取成功(无密模式)，IP: ${ip}:${port}`);
        }
        ...
        this.log(`❌ 获取代理失败，API返回: ${errorMsg}`);
        return false;
      }
    } catch (e) {
      this.log(`❌ 获取代理异常: ${e.message}`);
      return false;
    }
  }
