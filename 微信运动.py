...
class MiMotionRunner:
    def __init__(self, _user, _passwd):
        ...
        # 处理手机号格式（自动添加+86）
        if ("+86" not in user) and ("@" not in user):
            user = "+86" + user
        self.is_phone = "+86" in user
        self.user = user
    ...
    def get_app_token(self, login_token):
        ...
        try:
            ...
            self.log_str += f"获取app_token异常: {str(e)}\n"
            return None
    ...
    def login_and_post_step(self, min_step, max_step):
        ...
        self.log_str += f"已设置随机步数范围({min_step}~{max_step}) 随机值:{step}\n"
...
def run_single_account(total, idx, user_mi, passwd_mi, min_step, max_step):
    """单账号执行刷步数"""
    idx_info = f"[{idx+1}/{total}]" if idx is not None else ""
    log_str = f"[{format_now()}]\n{idx_info}账号：{desensitize_user_name(user_mi)}\n"
    exec_result = {"user": desensitize_user_name(user_mi), "success": False, "msg": ""}
    
    try:
        runner = MiMotionRunner(user_mi, passwd_mi)
        exec_msg, success = runner.login_and_post_step(min_step, max_step)
        log_str += runner.log_str + exec_msg + "\n"
        exec_result["success"] = success
        exec_result["msg"] = exec_msg
    except Exception as e:
        error_msg = f"执行异常: {str(e)}"
        log_str += error_msg + "\n" + traceback.format_exc() + "\n"
        exec_result["msg"] = error_msg
    
    print(log_str)
    return exec_result


def execute(accounts_info, sleep_seconds, use_concurrent):
    """批量执行多账号刷步数并推送结果"""
    exec_results = []
    ...
    if use_concurrent:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            futures = [executor.submit(
                run_single_account, total, idx, info["user"], info["pwd"], info["min_step"], info["max_step"]
        ...
        for idx, info in enumerate(accounts_info):
            result = run_single_account(
                total, idx, info["user"], info["pwd"], info["min_step"], info["max_step"]
            )
    ...
    summary = f"执行完成 | 总账号数：{total} | 成功：{success_count} | 失败：{total - success_count}"
    
    # 构造推送消息内容
    push_msg = f"【小米运动刷步数通知】\n{summary}\n\n详细结果：\n"
    for res in exec_results:
        status = "✅ 成功" if res["success"] else "❌ 失败"
        push_msg += f"- 账号：{res['user']} | {status} | 信息：{res['msg']}\n"
    
    # 使用notify推送结果
    try:
        notify.send('小米运动通知', push_msg)
        print(f"\nnotify推送成功：{summary}")
    except Exception as e:
        print(f"\nnotify推送失败：{str(e)}")
...
if __name__ == "__main__":
    ...
    # 1. 读取并解析环境变量misport
    if "misport" not in os.environ:
        print("未配置misport环境变量，无法执行")
        exit(1)
    
    misport_value = os.environ.get("misport")
    accounts_info = []
    # 支持多账号（用#分隔不同账号，每个账号内部用-分隔“账号-密码-最小步数-最大步数”）
    account_entries = misport_value.split('#')
    ...
    for idx, entry in enumerate(account_entries):
        ...
        parts = entry.split('-')
        if len(parts) != 4:
            print(f"第{idx+1}个账号格式错误（需为“账号-密码-最小步数-最大步数”），跳过该账号")
            continue
        user, pwd, min_step_str, max_step_str = parts
        # 校验步数是否为整数
        try:
            min_step = int(min_step_str.strip())
            max_step = int(max_step_str.strip())
            if min_step < 0 or max_step < min_step:
                print(f"第{idx+1}个账号步数范围无效（最小步数≥0且最大步数≥最小步数），跳过该账号")
                continue
            ...
            print(f"第{idx+1}个账号步数不是整数，跳过该账号")
            continue
    
    if not accounts_info:
        print("未解析到有效账号信息，无法执行")
        exit(1)
    
    # 2. 初始化其他参数（从原配置保留的通用参数）
    sleep_seconds = 5  # 多账号执行间隔默认5秒
    use_concurrent = False  # 默认不使用多线程（如需开启可改为True）
    
    # 3. 执行刷步数逻辑
    print(f"[{format_now()}] 开始执行小米运动刷步数 | 有效账号数：{len(accounts_info)}")
    execute(accounts_info, sleep_seconds, use_concurrent)