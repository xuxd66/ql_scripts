...
def get_min_max_by_time(hour=None, minute=None):
    ...
    min_step = get_int_value_default(config, 'MIN_STEP', 40000)
    max_step = get_int_value_default(config, 'MAX_STEP', 60000)
...
def push_to_push_plus(exec_results, summary):
    ...
    if PUSH_PLUS_TOKEN is not None and PUSH_PLUS_TOKEN != '' and PUSH_PLUS_TOKEN != 'NO':
        ...
        if len(exec_results) >= PUSH_PLUS_MAX:
            ...
            html += '</ul>'
        push_plus(f"小米刷步数通知 - {format_now()} ", html)
...
if __name__ == "__main__":
    ...
    if os.environ.__contains__("xmyd") is False:
        print("未配置xmyd变量，无法执行")
        exit(1)
    else:
        # region 初始化参数
        config = dict()
        try:
            config = dict(json.loads(os.environ.get("xmyd")))
        except:
            print("xmyd格式不正确，请检查Secret配置，请严格按照JSON格式：使用双引号包裹字段和值，逗号不能多也不能少")
            traceback.print_exc()
            exit(1)
